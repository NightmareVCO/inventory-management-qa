...
test.describe('Inventory Page for Employee User', () => {
    ...
    await page.getByPlaceholder('Search products...').fill('Test Product Employee');
    const rows = page.getByRole('row');
    await expect(rows).toHaveCount(2);
    await expect(page.getByRole('row', { name: /Test Product Employee/i })).toBeVisible();
  });

		test('filter by min price', async ({ page }) => {
			await page.getByLabel('Min Price').fill('10');

			await expect(page.getByRole('row', { name: /Test Product Employee/i })).toHaveCount(0);
		});
		...
		test('filter by low stock', async ({ page }) => {
			...
			await expect(page.getByRole('row', { name: /Test Product Employee/i })).toHaveCount(1);
		});
	...
	test('should allow editing products for employee users', async ({ page }) => {
		...
		await expect(page.getByLabel('Product Name*')).toHaveValue('Test Product Employee');
		await page.getByLabel('Product Name*').fill('Test Product Employee Edited');

		await expect(page.getByLabel('Description*')).toHaveValue('Test Description Employee');
		await page.getByLabel('Description*').fill('Test Description Employee Edited');
